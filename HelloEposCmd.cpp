...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId,
...
void PrintUsage() {
  ...
  cout << "\t-s   : protocol stack name (MAXON_RS232, CANopen, MAXON SERIAL "
          "V2, default - MAXON SERIAL V2)"
       << endl;
  cout << "\t-i   : interface name (RS232, USB, CAN_ixx_usb 0, CAN_kvaser_usb "
          "0,... default - USB)"
       << endl;
  cout << "\t-p   : port name (COM1, USB0, CAN0,... default - USB0)" << endl;
  cout << "\t-b   : baudrate (115200, 1000000,... default - 1000000)" << endl;
  cout << "\t-l   : list available interfaces (valid device name and protocol "
          "stack required)"
       << endl;
  cout << "\t-r   : list supported protocols (valid device name required)"
       << endl;
...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId,
  ...
  if (VCS_ActivateProfilePositionMode(p_DeviceHandle, p_usNodeId,
                                      &p_rlErrorCode) == 0) {
    ...
    positionList.push_back(54000);
    positionList.push_back(-108000);
    positionList.push_back(54000);

    //--------------------------------------------------------------------------
    unsigned int pProfileVelocity, pProfileAcceleration, pProfileDeceleration;
    if (VCS_GetPositionProfile(g_pKeyHandle, p_usNodeId, &pProfileVelocity,
                               &pProfileAcceleration, &pProfileDeceleration,
                               &p_rlErrorCode) == 0) {
      LogError("VCS_GetPositionProfile", lResult, p_rlErrorCode);
    }
    stringstream PositionProfile_msg;
    PositionProfile_msg << "Position Profile:\n";
    PositionProfile_msg << "Profile Velocity     =" << pProfileVelocity << '\n';
    PositionProfile_msg << "Profile Acceleration =" << pProfileAcceleration
                        << '\n';
    PositionProfile_msg << "Profile Deceleration =" << pProfileDeceleration
                        << '\n';
    LogInfo(PositionProfile_msg.str());
    //--------------------------------------------------------------------------
    bool absoluteMode = true, moveImmediatly = true;

    VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, 0, absoluteMode,
                       moveImmediatly, &p_rlErrorCode);
    usleep(1000000);
    //--------------------------------------------------------------------------

    for (list<long>::iterator it = positionList.begin();
      ...
      msg << "move to position = " << targetPosition
          << ", node = " << p_usNodeId;
      LogInfo(msg.str());
      if (VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, targetPosition,
                             absoluteMode, moveImmediatly,
                             &p_rlErrorCode) == 0) {
    ...
    if (lResult == MMC_SUCCESS) {
      ...
      if (VCS_HaltPositionMovement(p_DeviceHandle, p_usNodeId,
                                   &p_rlErrorCode) == 0) {
...
bool DemoProfileVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId,
                             unsigned int &p_rlErrorCode) {
  int lResult = MMC_SUCCESS;
  stringstream msg;
  ...
  if (VCS_ActivateProfileVelocityMode(p_DeviceHandle, p_usNodeId,
                                      &p_rlErrorCode) == 0) {
    ...
    for (list<long>::iterator it = velocityList.begin();
         it != velocityList.end(); it++) {
      long targetvelocity = (*it);

      stringstream msg;
      msg << "move with target velocity = " << targetvelocity
          << " rpm, node = " << p_usNodeId;
      LogInfo(msg.str());

      if (VCS_MoveWithVelocity(p_DeviceHandle, p_usNodeId, targetvelocity,
                               &p_rlErrorCode) == 0) {
        lResult = MMC_FAILED;
    ...
    if (lResult == MMC_SUCCESS) {
      ...
      if (VCS_HaltVelocityMovement(p_DeviceHandle, p_usNodeId,
                                   &p_rlErrorCode) == 0) {
        lResult = MMC_FAILED;
...
int MaxFollowingErrorDemo(unsigned int &p_rlErrorCode) {
  ...
  lResult =
      VCS_ActivateProfilePositionMode(g_pKeyHandle, g_usNodeId, &p_rlErrorCode);

  if (lResult) {
    lResult =
        VCS_SetMaxFollowingError(g_pKeyHandle, g_usNodeId, 1, &p_rlErrorCode);
  }

  if (lResult) {
    lResult = VCS_MoveToPosition(g_pKeyHandle, g_usNodeId, 1000, 1, 1,
                                 &p_rlErrorCode);
  }

  if (lResult) {
    lResult = VCS_GetDeviceErrorCode(g_pKeyHandle, g_usNodeId, 1,
                                     &lDeviceErrorCode, &p_rlErrorCode);
  }

  if (lResult) {
    lResult =
        lDeviceErrorCode == EXPECTED_ERROR_CODE ? MMC_SUCCESS : MMC_FAILED;
  }
...
int Demo(unsigned int *p_pErrorCode) {
  ...
  lResult = DemoProfilePositionMode(
      g_pKeyHandle, g_usNodeId,
      lErrorCode); // DemoProfileVelocityMode(g_pKeyHandle,
                   // g_usNodeId, lErrorCode);
...
int PrintAvailablePorts(char *p_pInterfaceNameSel) {
  ...
  do {
    if (!VCS_GetPortNameSelection(
            (char *)g_deviceName.c_str(), (char *)g_protocolStackName.c_str(),
            p_pInterfaceNameSel, lStartOfSelection, pPortNameSel, lMaxStrSize,
...
int PrintAvailableInterfaces() {
  ...
  do {
    if (!VCS_GetInterfaceNameSelection(
            (char *)g_deviceName.c_str(), (char *)g_protocolStackName.c_str(),
            lStartOfSelection, pInterfaceNameSel, lMaxStrSize, &lEndOfSelection,
            &ulErrorCode)) {
      lResult = MMC_FAILED;
...
int PrintDeviceVersion() {
  ...
  if (VCS_GetVersion(g_pKeyHandle, g_usNodeId, &usHardwareVersion,
                     &usSoftwareVersion, &usApplicationNumber,
                     &usApplicationVersion, &ulErrorCode)) {
    printf("%s Hardware Version    = 0x%04x\n      Software Version    = "
           "0x%04x\n      Application Number  = 0x%04x\n      Application "
           "Version = 0x%04x\n",
           g_deviceName.c_str(), usHardwareVersion, usSoftwareVersion,
           usApplicationNumber, usApplicationVersion);
    lResult = MMC_SUCCESS;
  }
...
int PrintAvailableProtocols() {
  ...
  do {
    if (!VCS_GetProtocolStackNameSelection(
            (char *)g_deviceName.c_str(), lStartOfSelection, pProtocolNameSel,
...
int main(int argc, char **argv) {
  ...
  /**
   * @brief Retrieve and print the current motor settings
   *VCS_GetEcMotorParameterEx(void* KeyHandle, unsigned short NodeId, unsigned
   int* pNominalCurrent, unsigned int* pMaxOutputCurrent, unsigned short*
   pThermalTimeConstant, unsigned char* pNbOfPolePairs, unsigned int*
   pErrorCode);
VCS_GetMotorType(void* KeyHandle, unsigned short NodeId, unsigned short*
pMotorType, unsigned int* pErrorCode);
    Configuration_DllExport int  VCS_GetEcMotorParameter(void* KeyHandle,
unsigned short NodeId, unsigned short* pNominalCurrent, unsigned short*
pMaxOutputCurrent, unsigned short* pThermalTimeConstant, unsigned char*
pNbOfPolePairs, unsigned int* pErrorCode);


   */
  //----------------------------------------------------------------------------
  // VCS_GetEcMotorParameterEx();
  BOOL getMotorType = 0;
  unsigned short pMotorType;
  unsigned int pErrorCode;
  getMotorType =
      VCS_GetMotorType(g_pKeyHandle, g_usNodeId, &pMotorType, &pErrorCode);
  stringstream motorType;
  motorType << "Motor Type:\n";
  motorType << (pMotorType == 1
                    ? "MT_DC_MOTOR"
                    : pMotorType == 10 ? "MT_EC_SINUS_COMMUTATED_MOTOR"
                                       : "MT_EC_BLOCK_COMMUTATED_MOTOR")
            << '\n';
  SeparatorLine();
  LogInfo(motorType.str());
  SeparatorLine();
  BOOL getECMotorParamsIsFault = 0;
  /*EX
  unsigned int pNominalCurrent, pMaxOutputCurrent;
  unsigned short pThermalTimeConstant;
  unsigned char pNbOfPolePairs;
  */
  // 	unsigned int pNominalCurrent, pMaxOutputCurrent;
  unsigned short pNominalCurrent, pMaxOutputCurrent, pThermalTimeConstant;
  unsigned char pNbOfPolePairs;
  //     Configuration_DllExport int  VCS_GetEcMotorParameter(void* KeyHandle,
  //     unsigned short NodeId, unsigned short* pNominalCurrent, unsigned short*
  //     pMaxOutputCurrent, unsigned short* pThermalTimeConstant, unsigned char*
  ...
  getECMotorParamsIsFault = VCS_GetEcMotorParameter(
      g_pKeyHandle, g_usNodeId, &pNominalCurrent, &pMaxOutputCurrent,
      &pThermalTimeConstant, &pNbOfPolePairs, &pErrorCode);
  if (getECMotorParamsIsFault != MMC_SUCCESS) {
    LogError("VCS_GetEcMotorParameterEx", getECMotorParamsIsFault, pErrorCode);
    return getECMotorParamsIsFault;
  }
  std::stringstream EcMotorParameters;
  EcMotorParameters << "EC Motor Paramenters: \n";
  EcMotorParameters << "Nominal Current[mA]      =" << pNominalCurrent << '\n';
  EcMotorParameters << "Max. Output Current   = " << pMaxOutputCurrent << '\n';
  EcMotorParameters << "Thermal Time Constant = " << pThermalTimeConstant
                    << '\n';
  EcMotorParameters << "N. of Pole Pairs 	  = "
                    << static_cast<uint8_t>(pMaxOutputCurrent) << '\n';

  LogInfo(EcMotorParameters.str());
  SeparatorLine();
  //----------------------------------------------------------------------------
